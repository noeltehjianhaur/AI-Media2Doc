...
- � **多语言转写**：自动识别视频原始语种并生成对应语言的转写文本。
- 🌐 **目标语言翻译**：可在设置中选择目标语言，将转写文本与生成内容一并翻译。
- 🗣️ **界面语言切换**：支持中英文界面一键切换。
- 🔗 **视频链接转写**：支持粘贴视频/网页链接（如小红书、YouTube、Facebook 等）进行转写。
...
2️⃣：参考`variables_template.env` 在项目根目录下生成 `variables.env` 文件。 请根据 [后端部署指引 / 配置项说明](./backend/README.md) 完善 `variables.env` 文件中的环境变量。
